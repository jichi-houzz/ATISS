...
def generate_from_test_set(network, config, split='test', num_scenes=10, device='cpu'):
    ...
    for idx in tqdm(range(len(test_dataset)), desc=f"Generating {split} scenes"):
        ...
        results.append((ground_truth, generated, scene_id, room_dims))
...
def save_results_to_npz(results, output_dir):
    ...
    for gt, gen, scene_id, room_dims in tqdm(results, desc="Saving"):
        ...
        create_comparison_image(gt, gen, scene_id, room_dims, vis_path)
...
def main():
    parser = argparse.ArgumentParser(description='Test inference with proper coordinate scaling')
    ...
    if args.verbose:
        for gt, gen, scene_id, room_dims in results[:3]:
