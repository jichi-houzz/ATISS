...
class BathroomDataPreprocessor:
    ...
    # Global normalization bounds (for cross-room learning)
    BOUNDS_MIN = -20.0  # meters
    BOUNDS_MAX = 20.0   # meters
    SIZE_MIN = 0.0      # meters
    SIZE_MAX = 20.0     # meters
    NORMALIZATION_VERSION = "v3_global_bounds"
    ...
    def normalize_coordinates(self, x: float, z: float, room_dims: Dict) -> Tuple[float, float]:
        """
        Normalize coordinates using global bounds (cross-room learning).
        
        Maps world coordinates to [0, 1] range using dataset-wide bounds.
        This allows the model to learn absolute positions across all rooms.
        
        Args:
            x: World x coordinate (meters)
            z: World z coordinate (meters)
            room_dims: Room dimensions dict (not used, kept for API compatibility)
            
        Returns:
            Normalized coordinates (x_norm, z_norm) in [0, 1]
        """
        x_norm = (x - self.BOUNDS_MIN) / (self.BOUNDS_MAX - self.BOUNDS_MIN)
        z_norm = (z - self.BOUNDS_MIN) / (self.BOUNDS_MAX - self.BOUNDS_MIN)
        return x_norm, z_norm

    def normalize_size(self, size_x: float, size_z: float, room_dims: Dict) -> Tuple[float, float]:
        """
        Normalize size using global bounds (rotation-invariant).
        
        Uses same bounds for both dimensions to maintain rotation invariance.
        Maps sizes to [0, 1] range using dataset-wide bounds.
        
        Args:
            size_x: Size in x direction (meters)
            size_z: Size in z direction (meters)
            room_dims: Room dimensions dict (not used, kept for API compatibility)
            
        Returns:
            Normalized sizes (size_x_norm, size_z_norm) in [0, 1]
        """
        size_x_norm = (size_x - self.SIZE_MIN) / (self.SIZE_MAX - self.SIZE_MIN)
        size_z_norm = (size_z - self.SIZE_MIN) / (self.SIZE_MAX - self.SIZE_MIN)
    ...
    def process_dataset(self, input_dir: str, output_dir: str):
        ...
        print(f"Using global bounds normalization:")
        print(f"  Coordinate bounds: [{self.BOUNDS_MIN}, {self.BOUNDS_MAX}] meters")
        print(f"  Size bounds: [{self.SIZE_MIN}, {self.SIZE_MAX}] meters")
        print(f"  Version: {self.NORMALIZATION_VERSION}\n")
