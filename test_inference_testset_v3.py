...
def load_architecture_from_json(json_path, room_dims):
    """
    ...
    center_x = room_dims['center_x']
    center_z = room_dims['center_z']
    ...
    for item in items:
        ...
        # Normalize coordinates (same as furniture)
        cx = item['center_x']
        cz = item['center_z']
        sx = item['size_x']
        sz = item['size_z']
...
def generate_from_test_set(network, config, json_dir, split='test', num_scenes=10, device='cpu'):
    ...
    for idx in tqdm(range(len(test_dataset)), desc=f"Generating {split} scenes"):
        ...
        # Load room_dims and architecture from metadata/JSON
        ...
        if json_dir and room_dims:
            ...
            architecture = load_architecture_from_json(json_path, room_dims)

            print(f"DEBUG: Loaded {len(architecture)} architecture items for {scene_id}")

        results.append((ground_truth, generated, scene_id, room_dims, architecture))
...
def create_comparison_image(ground_truth, generated, scene_id, room_dims, architecture, output_path):
...
def save_results_to_npz(results, output_dir):
    ...
    for gt, gen, scene_id, room_dims, architecture in tqdm(results, desc="Saving"):
        ...
        create_comparison_image(gt, gen, scene_id, room_dims, architecture, vis_path)
...
def main():
    ...
    if args.verbose:
        for gt, gen, scene_id, room_dims, arch in results[:3]:
