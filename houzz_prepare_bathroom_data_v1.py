...
class BathroomDataPreprocessor:
    ...
    def extract_floor_plan_info(self, items: List[Dict]) -> Tuple[np.ndarray, Dict]:
        ...
        if not floor_items:
            ...
            # Use floor to determine room size
            floor = floor_items[0]
            min_x = floor['center_x'] - floor['size_x']/2
            max_x = floor['center_x'] + floor['size_x']/2
            min_z = floor['center_z'] - floor['size_z']/2
            max_z = floor['center_z'] + floor['size_z']/2
    ...
    def create_floor_plan_mask(self, architecture_items: List[Dict], room_dims: Dict,
        ...
        """Create a binary floor plan mask."""
        mask = np.ones((resolution, resolution), dtype=np.uint8)

        # For simplicity, mark walls as occupied
        for item in architecture_items:
            if item['object_type'] == 'wall':
                # Convert wall position to mask coordinates
                x_norm, z_norm = self.normalize_coordinates(
                    item['center_x'],
                    item['center_z'],
                    room_dims
                ...
                mask[z_start:z_end, x_start:x_end] = 0
