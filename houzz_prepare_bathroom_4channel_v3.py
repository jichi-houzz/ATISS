...
class BathroomDataPreprocessor:
    ...
    # No normalization - save raw values in meters
    # ATISS will handle normalization using dataset.bounds
    ...
    def normalize_coordinates(self, x: float, z: float, room_dims: Dict) -> Tuple[float, float]:
        """
        Return raw world coordinates (no normalization).

        ATISS will normalize using dataset.bounds.

        Args:
            x: World x coordinate (meters)
            z: World z coordinate (meters)
            room_dims: Room dimensions dict (not used)

        Returns:
            Raw coordinates (x, z) in meters
        """
        return x, z

    def normalize_size(self, size_x: float, size_z: float, room_dims: Dict) -> Tuple[float, float]:
        """
        Return raw sizes (no normalization).
        ...
        Returns:
            Raw sizes (size_x, size_z) in meters
        """
    ...
    def process_dataset(self, input_dir: str, output_dir: str):
        ...
        print(f"No normalization - saving raw values in meters")
        print(f"ATISS will normalize using dataset.bounds:")
        print(f"  Position: [-25, 25] meters")
        print(f"  Size: [0, 7] meters")
        print(f"  Angle: degrees → radians (cos, sin)\n")
